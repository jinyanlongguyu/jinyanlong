...
def gen_annual_report_template_bytes() -> bytes:
    ...
    ws1.cell(row=1, column=1, value="武汉金艳龙科技有限公司 · 2025年度年报数据导入模板").font = Font(name="微软雅黑", bold=True, size=14, color="1E3A8A")
...
def gen_annual_report_template_pdf_bytes() -> bytes | None:
    ...
    try:
        ...
        pdf.cell(0, 12, "武汉金艳龙科技有限公司", ln=True, align="C")
        pdf.ln(4)
        if use_cjk:
...
st.title("💰 金艳龙AI税务助手")
st.caption("武汉金艳龙科技有限公司 · 一站式企业税务计算与AI申报指引")

tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs(["🗂️ 年报导入", "📊 季度申报", "💰 工资计算", "📋 批量导入", "🏦 税款缴纳清单", "♿ 残保金", "📄 申报说明", "📖 申报指南"])
...
with tab3:
    ...
    for i in range(num_emp):
        ...
        with c1:
            name = st.text_input(
                "姓名", value=default_name, key=f"name_{i}"
            )
    ...
    if st.button("🚀 开始计算", use_container_width=True, type="primary"):
        ...
        with dl_col2:
            ...
            if pdf_bytes:
                st.download_button(
                    label="📥 下载申报底稿（PDF）",
                    data=pdf_bytes,
                    file_name=f"申报底稿_{datetime.now().strftime('%Y%m')}.pdf",
        ...
        with st.expander("🔎 工资数据校验（银行流水 vs 个税申报 vs 年报）", expanded=False):
            ...
            if st.button("🔍 开始校验", key="run_salary_val", use_container_width=True):
                # 准备员工数据
                emp_list = []
                ...
                if bank_file_val:
                    try:
                        ...
                        st.success(f"✅ 银行流水读取 {len(bank_df)} 条")
                    except Exception as e:
                ...
                if tax_file_val:
                    try:
                        ...
                        st.success(f"✅ 个税申报记录读取 {len(tax_df)} 条")
                    except Exception as e:
                ...
                if bank_df is not None:
                    ...
                    if bm:
                        ...
                        if bm["match"]:
                            ...
                            st.error(f"⚠️ 差异较大！建议检查银行摘要关键词或系统工资录入")
                    else:
...
with tab7:
    ...
    if "results" not in st.session_state:
        st.info("💡 请先在「💰 工资计算」页面录入员工信息并点击「开始计算」，再返回本页查看 AI 生成的申报说明。")
...
武汉金艳龙科技在职职工 ≤ 30人，符合免征条件，只需进行「零申报」即可。
...
================================================================"""
    ...
    with dl_g1:
        st.download_button(
            label="📥 下载完整申报指南（TXT）",
            data=full_guide,
            file_name=f"税务申报操作指南_武汉金艳龙科技_{datetime.now().strftime('%Y%m%d')}.txt",
            mime="text/plain",
            use_container_width=True,
        )
    with dl_g2:
        pdf_bytes = make_pdf(
            "税务申报操作指南 - 武汉金艳龙科技有限公司",
            full_guide.split("\n"),
            ""
        )
        if pdf_bytes:
            st.download_button(
                label="📥 下载完整申报指南（PDF）",
                data=pdf_bytes,
                file_name=f"税务申报操作指南_武汉金艳龙科技_{datetime.now().strftime('%Y%m%d')}.pdf",
                mime="application/pdf",
                use_container_width=True,
            )
...
with tab6:
    ...
    st.caption("武汉金艳龙科技有限公司 · 小规模纳税人 · 小型微利企业")
...
""")
    ...
    with st.expander("Q1：武汉金艳龙科技需要缴纳残保金吗？"):
        st.markdown(f"""
**答：不需要（免征）。**

武汉金艳龙科技有限公司在职职工 **{def_prev_employees} 人 ≤ 30人**，符合小微企业免征条件（发改价格规〔2019〕2015号）。
...
================================================================"""
    ...
    with dl_f1:
        st.download_button(
            ...
            data=fund_report,
            file_name=f"残保金测算报告_{def_year}年_武汉金艳龙科技.txt",
            mime="text/plain",
            use_container_width=True,
        )
    with dl_f2:
        ...
        if pdf_bytes:
            st.download_button(
                ...
                data=pdf_bytes,
                file_name=f"残保金测算报告_{def_year}年_武汉金艳龙科技.pdf",
                mime="application/pdf",
                use_container_width=True,
            )
...
with footer_col2:
    st.caption(
        f"© {datetime.now().year} 金艳龙AI税务助手 v1.6 · "
        "武汉金艳龙科技有限公司 · "
        "基于小规模纳税人+小型微利企业场景 · "
        "仅供参考，以税务机关最新公告为准"
    )
